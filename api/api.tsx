...
import { ApiProps, ApiState, SDK } from './types';
...
export const Api = React.memo(function Api ({ children, endpoints, store }: Props): React.ReactElement<Props> | null {
  ...
  const initSDK = useMemo(() => async (api: ApiPromise) => {
    await api.isReady;
    const wallet = new WalletPromise(api);

    setSDK({ wallet: wallet });
  }, [setSDK]);
  ...
  useEffect((): void => {
    ...
    api = new ApiPromise(options({ provider, registry, types: {} }));
